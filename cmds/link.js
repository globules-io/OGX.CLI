        ...
        if(index){
            if(['all', 'controllers'].includes(args[0])){               
                fs.readdirSync('www/js/controllers').forEach(file => {  
                    if(/\.js$/.test(file)){
                        if(!index.includes('<script type="application/javascript" src="js/controllers/'+file+'">')){                
            ...
            if(['all', 'views'].includes(args[0])){               
                fs.readdirSync('www/js/views').forEach(file => {   
                    if(/\.js$/.test(file)){   
                        if(!index.includes('<script type="application/javascript" src="js/views/'+file+'">')){                            
                            index = index.replace('</head>', '<script type="application/javascript" src="js/views/'+file+'"></script>\n</head>');  
                        }
                        if(!index.includes('<link rel="stylesheet" href="css/views/view.'+file.split('.')[1]+'.css">')){     
            ...
            if(['all', 'stages'].includes(args[0])){   
                fs.readdirSync('www/js/stages').forEach(file => {   
                    if(/\.js$/.test(file)){
                        if(!index.includes('<script type="application/javascript" src="js/stages/'+file+'">')){                              
                            index = index.replace('</head>', '<script type="application/javascript" src="js/stages/'+file+'"></script>\n</head>');  
                        }
                        if(!index.includes('<link rel="stylesheet" href="css/stages/stage.'+file.split('.')[1]+'.css">')){            
            ...
            if(['all', 'css'].includes(args[0])){   
                fs.readdirSync('www/css/bin').forEach(file => {   
                    if(/\.css$/.test(file)){
                        if(!index.includes('<link rel="stylesheet" href="css/bin/'+file+'">')){                           
            ...
            if(['all', 'js'].includes(args[0])){   
                fs.readdirSync('www/js/bin').forEach(file => {   
                    if(/\.js$/.test(file)){
                        if(!index.includes('<script type="application/javascript" src="js/bin/'+file+'">')){                              
